...
def callGitHubCLIdiff(strCmd):
    ...
    # GitHub CLIの呼び出し
    try:
        procObj = subprocess.run(['cmd', '/c', strCmd], stdout = subprocess.PIPE)
        outputStr = procObj.stdout.decode("utf-8")
    except:
        print('GitHub CLIにてエラーが発生しました')
        exit(1)
...
def callGitHubCLIcomment(prNumber, comment):
    ...
    try:
        # ToDo: bodyに空白が入ってもセキュリティ的に shell=True を使わない方法を検討
        # 空白が入ると body の後ろが次の引数になる
        procObj = subprocess.run(strCmd, shell=True, stdout = subprocess.PIPE)
        # procObj = subprocess.run(['cmmd', '/c', strCmd], stdout = subprocess.PIPE)
        outputStr = procObj.stdout.decode("utf-8")
    except:
        print('GitHub CLIにてエラーが発生しました')
        return False
...
def main(args):
    """メイン関数
        チェック結果をJSON形式でoutput.jsonに保存する
    Args:
    ...
    for item in strResult:
        ...
        if 'protectedMaterialAnalysis' in jsonAzureResult and 'codeCitations' in jsonAzureResult['protectedMaterialAnalysis'] and len(jsonAzureResult['protectedMaterialAnalysis']['codeCitations']) > 0:
            ...
            item.update(
                {
                    "matchSnippet": matchCode["match"] if matchCode is not None else []
                }
            )
    ...
    with open('../output.json', 'w', encoding='utf-8') as json_file:
        json.dump(strResult, json_file, ensure_ascii=False, indent=4)

    print("チェック完了、JSONファイル(../output.json)に保存しました")
...
if __name__ == "__main__":
    main(sys.argv[1:])  # コマンドライン引数（スクリプト名を除く）を渡す
